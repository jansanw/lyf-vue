                ...
                <div class="">
                    <ul class="order-list" v-if="order_list.length>0">
                        <li v-for="order in order_list" style="margin-bottom:.27rem;" class="aui-border-b">
                                        ...
                                        <div class="state">
                                            <div class="state-cont">
                                                <p class="h" style="width:auto;" v-if="!order.if_lock">
                                                    {{order.order_state|order_stae}}</p>
                            ...
                            <div class="module" @click="go_order_detail(order.order_id)"
                                 v-for="goods in order.order_goods" style="border-bottom:1px solid #ffffff;">
                            ...
                            <div class="module pay">
                                <div class="o-total-price">
                                    <div class="cont"><span>共<b>{{order.goods_count}}</b>件商品</span> <span>合计:<b>￥{{order.order_amount}}</b></span>
                                        <span>(含运费<b>￥{{order.shipping_fee}}</b>)</span></div>
                                </div>
                            </div>
                            <div class="module orderop" v-if="order.order_state !=20">
                                <div class="o-tab-btn">
                                    <ul>
                                        <li class="h" v-if="order.order_state==30"
                                            @click="order_receive(order.order_id)"> 确认收货
                                        </li>
                                        <li class="h" v-if="order.order_state==40"
                                            @click="$router.push({name:'order_rate',params:{order_id:order.order_id}})">
                                            评价订单
                                        </li>
                                        <li class="" v-if="order.order_state==30 || order.order_state==40"
                                            @click="$router.push({name:'order_logistics',params:{order_id:order.order_id}})">
                                            查看物流
                                        </li>
                                        <!--li class="h" v-if="order.order_state==10"> 立即付款 </li-->
                                        <li class="" v-if="order.order_state==10" @click="order_cancel(order.order_id)">
                                            取消订单
                                        </li>
