    ...
    <div class="page">
        <scroll class="page-content" ref="lyf_scroll" :on-refresh="onRefresh" :on-infinite="onInfinite" v-show="init"
            ...
            <div style="background-color: #fff;" v-for="item in data">
                <div class="aui-border-b hm-flex"
                     style="padding: 10px;text-align: right;justify-content: space-between;">
                    ...
                    <div class="hm-flex-4 hm-flex"
                        ...
                        <div>卖家：{{item.store_name}}
                            <div class="aui-label aui-label-danger"
                                 @click="go_return_ship(item.refund_id,item.admin_state_v)"
                                 v-if="item.seller_state_v==2 && item.admin_state_v<2" style="float:right">上传退货凭证
                            </div>
                        </div>
                    </div>
                </div>
