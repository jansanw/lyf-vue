...
<template lang="html">
    <mt-popup v-model="popupVisible_o" position="bottom" style="width:100%; height:10.8rem;">
                ...
                <li v-for="item in voucherlist">
                    <div class="voucher-list-item">
                        <div class="voucher-list-item-left">
                            <p style="font-size:.67rem;color:#EA5A49;">
                                <small style="font-size:.43rem; margin-right:3px;">￥</small>
                                {{item.voucher_t_price}}
                            </p>
                            <p class="text-12 color-royal">订单满<strong>{{item.voucher_t_limit}}</strong>元使用（不含邮费）</p>
                            <p class="text-12 color-royal">使用期限：{{item.voucher_t_start_date}} -
                                {{item.voucher_t_end_date}}</p>
                        </div>
                        <div class="" style="width:2.13rem;align-self:center;text-align:right">
                            <button class="button button-royal button-small button-outline color-royal"
                                    style="width:1.47rem;" v-if="!item.get_num">已领取
                            </button>
                            <button v-else @click="get_voucher(item.voucher_t_id)" style="width:1.47rem;"
                                    class="button button-assertive button-outline button-small">领取
                            </button>
                        </div>
                    </div>
                </li>
        ...
        <div @click="close_voucher()"
             style="height:1.33rem; line-height:1.33rem; text-align:center;color:#ffffff;background-color:#EA5A49; font-size:.37rem;">
            关闭
        </div>
    </mt-popup>
